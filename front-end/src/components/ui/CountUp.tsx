...
export default function CountUp({
  ...
  const startValue = direction === 'down' ? Math.max(from, to) : from;
  const endValue = direction === 'down' ? Math.min(from, to) : to;
  ...
  const formatValue = useCallback(
    (latest: number) => {
      ...
      const value = hasDecimals ? latest : Math.round(latest);
  ...
  }, [isInView, startWhen, motionValue, endValue, delay, onStart, onEnd, duration]);
